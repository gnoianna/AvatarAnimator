...
def detect_face_and_landmarks(img):
    ...
    return status, img, facial_landmarks
...
def draw(img, triangle_list, color):
    t = triangle_list
    cv2.line(img, (int(t[0]), int(t[1])), (int(t[2]), int(t[3])), color, 1, cv2.LINE_AA, 0)
    cv2.line(img, (int(t[2]), int(t[3])), (int(t[4]), int(t[5])), color, 1, cv2.LINE_AA, 0)
    cv2.line(img, (int(t[4]), int(t[5])), (int(t[0]), int(t[1])), color, 1, cv2.LINE_AA, 0)


def draw_triangles(src_img, src_points, dst_img, dst_points):
    for src_triangle_points, dst_triangle_points in delaunay_triangulation(src_points, dst_points):
        ...
        draw(src_img, t_s, (255, 0, 0))
        draw(dst_img, t_d, (255, 255, 255))
        ...
        if animate:
            if success:
                ...
                if rects:
                    ...
                    animate = not animate
                    _, avatar_original_img, avatar_shape = detect_face_and_landmarks(avatar_original_img)
...
def image():
    if request.method == "POST":
        ...
        if src_file and avatar_file and allowed_file(src_file.filename) and allowed_file(avatar_file.filename):
            ...
            src_status, src_img, src_points = detect_face_and_landmarks(src_img)
            avatar_status, avatar_img, avatar_points = detect_face_and_landmarks(avatar_img)
