...
global detector, predictor, avatar_original, avatar_points_original
...
def animate(src_img, avatar_img):
...
def all_inside(src_img, src_points):
    global avatar_original, avatar_points_original
    avatar_new_face = np.zeros(src_img.shape, np.uint8)
    avatar_img_copy = np.copy(avatar_original)

    for i, indices in enumerate(app.gen_triangles_indices(src_points)):
        avatar_triangle_points = app.gen_xy_from_indices(avatar_points_original, indices)
        src_triangle_points = app.gen_xy_from_indices(src_points, indices)

        avatar_triangle, avatar_img_cropped, _ = app.gen_cropped_triangle(avatar_original, avatar_triangle_points)
        _, avatar_img_cropped_copy, _ = app.gen_cropped_triangle(avatar_img_copy, avatar_triangle_points)
        src_triangle, src_img_cropped, src_b_rect = app.gen_cropped_triangle(src_img, src_triangle_points)
    ...
    transform = PiecewiseAffineTransform()
    transform.estimate(avatar_points_original[0:27], src_points[0:27])
    face = warp(avatar_new_face, transform, output_shape=avatar_original.shape, order=1, mode='wrap')
    mask = warp(src_img, transform, output_shape=avatar_original.shape, order=1, mode='wrap')

    face = cv2.normalize(face, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)
    mask = cv2.normalize(mask, None, 0, 255, cv2.NORM_MINMAX, cv2.CV_8U)

    avatar_original = avatar_original & mask


    avatar_original += face
...
def generate():
    ...
    src_without_landmarks, src_points = app.gen_landmarks(src_img, 0)
    avatar_original, avatar_points_original = app.gen_landmarks(avatar_img, 0)
    # transform_avatar(avatar_without_landmarks, avatar_points, src_without_landmarks,
    #                  src_points)

    all_inside(src_without_landmarks, src_points)
    #avatar_original = src_img
    plt.figure()
    cv2.imshow("ANIMATED", avatar_original)
